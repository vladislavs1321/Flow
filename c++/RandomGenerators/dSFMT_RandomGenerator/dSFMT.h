...
// Addition by Shingariov Igor
// ----------------------------------------------------------------------------
// The normal random values generator based on dSFMT_RandomGenerator
// Based on NormRandG from Randoms.cpp

/* Before using this generator one should declare
    int seed = 12345;   // Initial value of generator (set any int value)
    dsfmt_t dsfmt;      // (global variable) Structure for generator's functioning
    dsfmt_init_gen_rand(&dsfmt, seed); // Initialize generator     */

double dSFMT_NormRandGenerator(dsfmt_t *dsfmt, double dM, double dD);
// This generator is described in Numerical Recipes in C
//-----------------------------------------------------------------------------
unsigned int dSFMT_PoissonRandValue(dsfmt_t *dsfmt, double a);
// Poisson value generator
#endif
