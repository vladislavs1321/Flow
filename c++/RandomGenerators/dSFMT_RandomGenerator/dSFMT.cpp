...
// mechanism of static variables
 ...
 do
  {
   dUVal1 = 2.0*dsfmt_genrand_close_open(dsfmt)-1.0; // pick two uniform numbers in the square extending from -1 to +1 in each direction
   dUVal2 = 2.0*dsfmt_genrand_close_open(dsfmt)-1.0;
...
// This function generates the Poisson random value
// 
  double p = exp(-a);
  double s = dsfmt_genrand_close_open(dsfmt)-p;
  double i = 0.0;
